...
-- Notes:
-- - Requires CREATE USER (run as root for VM): mariadb -uroot -prootpassword < esport_ddl3.sql
...
-- - Also GRANTs the VM app user (user/localhost) full access
...
-- VM MariaDB app account (must match db_connect.php)
...
GRANT SELECT, INSERT ON EsportLeagueDB.Players TO 'coach_role'@'localhost';
GRANT SELECT, UPDATE ON EsportLeagueDB.Coaches TO 'coach_role'@'localhost';
GRANT SELECT, DELETE, INSERT ON EsportLeagueDB.Teams TO 'coach_role'@'localhost';
GRANT SELECT, INSERT, UPDATE ON EsportLeagueDB.TeamMembers TO 'coach_role'@'localhost';
